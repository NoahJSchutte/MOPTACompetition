...
class Stochastic3Stage(Algorithm):
    def __init__(
            self,
    ):
        super().__init__()
    ...
    def build_stage1(self,timeout=INFINITY, ):
        ...
        stage_model.addConstrs((x[i, block] == 0 for i in self.SURGERIES for block in self.BLOCKS if b[i, block] == 0), '3b')
    ...
    def build_stage2(self,x,timeout=INFINITY,):
        ...
        #Objective
        #overtime and idletime cost        
        stage_model.setObjective(self.COSTS["IDLETIME"]*CI_ 
                            + self.COSTS["OVERTIME"]*CO_ 
                            , GRB.MINIMIZE)
        ...
        return {"model":stage_model,"y":y}


    def build_block(self,x,y,block,timeout=INFINITY):
        ...
        q = block_model.addVars(SAE,self.SCENARIOS, vtype=GRB.BINARY, name='q') #precedence constraint between surgery pairs
        p = block_model.addVars(block_surgeries,block_surgeries,vtype=GRB.BINARY,name='p')#precedence between electives
        z = block_model.addVars(block_surgeries, self.SCENARIOS, vtype=GRB.BINARY, name='z') # Elective cancelled or not {0,1}
        ...
        CC = block_model.addVar(vtype=GRB.CONTINUOUS, name='CC') #cancelling
        ...
        block_model.addConstrs((T[j,w]  >= (T[i,w] + self.SURGERY_DURATIONS[i][w])*q[i,j,w]
        ...
        block_model.addConstrs( (q[i,j,w] + q[j,i,w] == x[i,block] * (1-z[i,w]) * x[j,block] 
                                                    for i in block_surgeries 
                                                    for j in block_surgeries                                                                 
                                                    for w in self.SCENARIOS
                                                    if i != j)  )
        block_model.addConstrs( p[i,j] == q[i,j,w]  for i in block_surgeries 
                                                    for j in block_surgeries                                                                 
                                                    for w in self.SCENARIOS
                                                    if i != j) 
        #ordering of electives consistent across scenarios

        block_model.addConstrs( q[i,j,w] + q[j,i,w]  == x[i,block] * (1-z[i,w]) *  y[j,block,w] 
                                                    for i in block_surgeries 
                                                    for j in block_emergencies                                                   
                                                    for w in self.SCENARIOS
                                                    if i != j  )
        
        block_model.addConstrs( q[i,j,w] + q[j,i,w]  == y[i,block,w] * y[j,block,w] 
                    ...
                    for w in self.SCENARIOS:
                        if self.emergency_arrival_map["TIME"][i][w] < self.emergency_arrival_map["TIME"][j][w]:
                            if y[i,block,w] and y[j,block,w]:
                                block_model.addConstr(q[i,j,w]  == 1)
                                block_model.addConstr(q[j,i,w]  == 0)

        block_model.addConstrs ( q[i,k,w] >= q[j,k,w]*q[i,j,w] 
                                    for i in SA
                                    for j in SA
                                    for k in SA
                                    for w in self.SCENARIOS
        ...
        * 1/(len(self.SCENARIOS))), 'emergencywaiting') 
        ...
        #cancellation cost
        block_model.addConstr((CC == quicksum(z[i,w] for i in block_surgeries for w in self.SCENARIOS) * 1/(len(self.SCENARIOS))), 'cancellation') 
        ...
        return {"model":block_model,"q":q,"s":s,"z":z,"T":T}

    def build_stage3(self,x,y,timeout=INFINITY):
        block_results = {}
        for block in self.BLOCKS:
            block_results[block] = self.build_block(x,y,block,timeout)
        return block_results
            

    def build_model(self, csv_output, timeout=[INFINITY,INFINITY,INFINITY], threads=0, node_file_start=0, node_dir=''):
        ...
        block_results  = self.build_stage3(x,y,timeout[2])

        s = {}
        T = {(i,w):"-" for i in set(self.SURGERIES).union(self.EMERGENCY_SURGERIES) for w in self.SCENARIOS}
        z = {}
                ...
                for i in self.SURGERIES:
                    if x[i,block]:
                        ...
                        for w in self.SCENARIOS:
                            ...
                            z[i,w] = block_results[block]["z"][i,w].X
                ...
                for i in self.EMERGENCY_SURGERIES:
                    for w in self.SCENARIOS:
                        if y[i,block,w] and any([self.emergency_duration_map[i][v]>0 for v in self.SCENARIOS]):
                            T[i,w] = block_results[block]["T"][i,w].X
            # else:
            #     print(f"block {block} has no solution")
            #     sys.exit()
        self.write_results(stage_results,block_results,x,s,y,z,T,csv_output)


    def write_results(self,stage_results,block_results,x,s,y,z,T,csv_output):
        results = []
        assigned = 0
        for i in self.SURGERIES:
